...
### Updates
- 4.16.2026. I added a `requirements.txt` specifying exact package versions. As the project has aged, some newer library updates have deprecated old methods we still use in charsiu, leading to compatibility issues. Through testing, I found the project supports up to Python 3.9 max, so this configuration is tailored to guarantee a smooth setup.
...
This project requires the TIMIT dataset for training. Due to open-source license restrictions, the dataset cannot be included directly in this repository. Furthermore, the official source has deprecated the remote download method; the dataset must now be downloaded manually from the web (purchase required) for local use. Accordingly, the remote download code in this project has been replaced to only support local data loading. Please download and extract the dataset to the project's root directory to get started. 
